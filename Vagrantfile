...
Vagrant.configure(2) do |config|
  ...
  if File.exists?(File.join(File.dirname(__FILE__), 'custom-setup.sh')) then
    config.vm.provision :shell, :path => File.join( File.dirname(__FILE__), 'custom-setup.sh' ), run: "always"
  end
end
